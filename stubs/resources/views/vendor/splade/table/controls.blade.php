<div class="flex flex-row sm:justify-start mb-3 px-4 sm:px-0">

    <div class="order-1 mr-0 sm:mr-2">
        @stack('button')
        @stack('tools')
    </div>

    @if($table->hasExports() || $table->hasBulkActions())
        <div class="order-2 mr-2 sm:mr-2" v-if="table.hasSelectedItems || @js($table->hasExports())">
    ...
    @if($table->hasFilters())
        <div class="order-4 mr-2 sm:mr-2">
            @include('splade::table.filters')
        </div>
    @endif

    @if($table->searchInputs('global'))
        <div class="order-5 flex-grow mr-2 sm:mr-2">
