<thead class="bg-gray-50 dark:bg-slate-900">
    <tr>
        @if($hasBulkActions = $table->hasBulkActions())
            <th width="32" class="px-3 py-2 text-xs">
        ...
        @foreach($table->columns() as $column)
            <th
                ...
                class="@if($loop->first && $hasBulkActions) px-3 @else px-3 @endif border-l py-2 text-left text-xs font-medium tracking-wide text-gray-500 {{ $column->classes }}"
            >
                @if($column->sortable)
