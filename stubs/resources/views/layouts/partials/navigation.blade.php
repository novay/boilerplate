    ...
    <div id="application-sidebar" class="hs-overlay hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed top-0 left-0 bottom-0 z-[60] w-56 bg-white border-r border-gray-200 pt-4 pb-0 overflow-y-auto scrollbar-y lg:block lg:translate-x-0 lg:right-auto lg:bottom-0 dark:scrollbar-y dark:bg-gray-800 dark:border-gray-700">
        ...
        <nav class="hs-accordion-group p-3 w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
            <ul class="space-y-0.5">
                ...
                <li>
                    <x-splade-link :href="route('panel.index')" class="flex items-center gap-x-1.5 px-2 py-1.5 text-sm font-medium text-slate-700 rounded hover:bg-gray-100 {{ request()->routeIs('panel.index') ? 'bg-gray-100 dark:bg-gray-900 dark:text-white' : 'dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300' }}">
                        <Icon class="w-5 h-5" icon="icon-park-twotone:home" />
                        Dashboard
                    </x-splade-link>
                </li>
                <li>
                    <x-splade-link :href="route('panel.users.index')" class="flex items-center gap-x-1.5 px-2 py-1.5 text-sm font-medium text-slate-700 rounded hover:bg-gray-100 {{ request()->routeIs('panel.users.*') ? 'bg-gray-100 dark:bg-gray-900 dark:text-white' : 'dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300' }}">
                        <Icon class="w-5 h-5" icon="icon-park-twotone:address-book" />
                        Pengguna
                    </x-splade-link>
                </li>
                <li class="hs-accordion" id="kbud-accordion">
                    ...
                    <div id="kbud-accordion-sub" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300 hidden">
                        <ul class="hs-accordion-group" data-hs-accordion-always-open>
                            <li>
                                <a href="javascript:;" class="flex items-center gap-x-2 px-2.5 py-1.5 text-sm font-medium text-slate-700 rounded hover:bg-gray-100 dark:bg-gray-800 dark:text-slate-400 dark:hover:text-slate-300 pl-3">
                                    Menu 2
                                </a>
                            </li>
                        </ul>
